...
class TrapezoidIntegrator(private val settings: IntegrationParameters) : Integrator {
    ...
    private fun nextIntegralApproximation(
        ...
        repeat(divisions - 1) {
            integralSum += function(currentParam)
            currentParam += step
        }

        val resultSum = ((function(start) + function(endInclusive)) / 2 + integralSum) * step
